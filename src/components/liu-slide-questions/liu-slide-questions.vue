...
export default defineComponent({
	...
	setup(props, context) {
		const topBanner = ref('/static/images/questions/banner.png')
		const chooseonImg = ref('/static/images/questions/chooseon.png')
		const chooseonImg2 = ref('/static/images/questions/chooseon2.png')
		const newQuestionsAnswer = reactive<any>([])
		const formSubmitData = reactive<any>([]) // 提交所需数据
		const subState = ref(true) // 防止重复提交
		const totalNum = ref(0)
		let currentIndex = ref(0)

		const submitData = () => {
			console.log("打印提交数据==>", formSubmitData)
			// 防重复提交校验
			// if (!this.subState) {
			// 	return
			// }
			//答题结果校验
			for (let i = 0; i < formSubmitData.length; i++) {
				if (!formSubmitData[i].userAnswer) {
			...
			context.emit("submit", formSubmitData)
			// 复用时修改  提交答题结果请求接口
		...
		const getQuestionList = () => {
			...
			// 例如： totalNum.value = res.data.length
			//       newQuestionsAnswer = res.data
			//       setEmptyData()
		}
		...
		const setEmptyData = () => {
			for (let i = 0; i < totalNum.value; i++) {
				...
				let arr:FormSubmitItem = {
					questions: props.questionsAnswer[i] as string, //题目内容
					userAnswer: '', //用户选择的选项文本
				}
				formSubmitData.push(arr)
			}
		}
		//单选事件
		const singChoose = (j: any, e: any) => {
			if (newQuestionsAnswer[j].children[e].ifselect) {
				newQuestionsAnswer[j].children[e].ifselect = 0
				formSubmitData[j].userAnswer = ''
			} else {
				for (var i = 0; i < newQuestionsAnswer[j].children.length; i++) {
					if (newQuestionsAnswer[j].children[i].ifselect) {
						newQuestionsAnswer[j].children[i].ifselect = 0
					}
				}
				newQuestionsAnswer[j].children[e].ifselect = 1

				// 这块结合业务逻辑可以进行优化处理
				// 例如  只保存用户已选择答案Id等
				formSubmitData[j].userAnswer = newQuestionsAnswer[j].children[e]
			}
			newQuestionsAnswer.value = JSON.parse(JSON.stringify(newQuestionsAnswer))
		}
		//多选事件
		const multyChoose = (j: any, e: any) => {
			newQuestionsAnswer[j].children[e].ifselect = newQuestionsAnswer[j].children[e].ifselect ^ 1
			let obj:any[] = []
			for (let i = 0; i < newQuestionsAnswer[j].children.length; i++) {
				if (newQuestionsAnswer[j].children[i].ifselect) {
					// 这块结合业务逻辑可以进行优化处理
					// 例如  只保存用户已选择答案Id等
					// obj.push(this.newQuestionsAnswer[j].children[i].id)

					obj.push(newQuestionsAnswer[j].children[i])
				}
			}
			// this.formSubmitData[j].userAnswer = obj.toString()
			formSubmitData[j].userAnswer = obj
		}
		//富文本
		const bindTextAreaBlur = (index) => {
			formSubmitData[index].userAnswer = newQuestionsAnswer[index].answer
		}
		//swiper改变时 的事件
		const eventHandle = (e) => {
		...
		onMounted(() => {
			newQuestionsAnswer.push(...props.questionsAnswer)
			totalNum.value = props.questionsAnswer.length
			setEmptyData()
		})
		return {
			topBanner,
