	...
	<view class="pyh-nv-box" ref="pyhNvBox" :style="style" v-if="show">
		...
		<view :class="['nvBox',{'noModel':!nvConfig.model}]" :style="[{'width':windowInfo.width+'px'},{'color':getTxtColor},{'background':getBgColor},{'opacity':nvConfig.transparent&&nvConfig.transparent.type=='content'?transparent.opacity:1},{'position':isFixed?'fixed':'relative'},{'top':(isFixed&&nvConfig.model?(nvBoxRect.top+'px'):0)}]" v-if="nvConfig.model?!nvBoxRect.wait:true">
			...
			<view class='nvFixed' :style="[{'width':windowInfo.width+'px'}]">
				<!-- slot内嵌 -->
				<view :class="[{'androidwx':androidwx},
				...
				{'nvContentBottom':nvConfig.componentsFlex&&nvConfig.componentsFlex=='bottom'}
					...
					<block v-if="nvConfig.address">
						...
						<view v-else :class="['nvAddress','nvSearchAddress',{'nvSBoxFlex':nvConfig.componentsFlex&&(nvConfig.componentsFlex=='top'||nvConfig.componentsFlex=='bottom')}]" :style="[{'width':nvConfig.address.width||'144rpx'},{'background':(nvConfig.address.bgColor||'')},{'margin-right':Boolean(nvConfig.address.bgColor||'')?'20rpx':'0'},{'color':(nvConfig.address.color||'')}]" @tap.stop="nvAddressTap">
				...
				<!-- 返回键 -->
				<block v-if="!(nvConfig.back&&nvConfig.back.hide)&&!nvConfig.address">
					<image :class="['nvback','nvbackImg']" @tap.stop="nvBackTap" :src="isSharePage&&!(nvConfig.back&&nvConfig.back.iconForce)&&nvConfig.back.icon.homePath?nvConfig.back.icon.homePath:nvConfig.back.icon.path" :style="nvConfig.back.icon.style||''" mode="aspectFill" v-if="nvConfig.back.icon&&nvConfig.back.icon.path"></image>
					<text :class="['iconfont','nvback',{'nvhome':isSharePage&&!(nvConfig.back&&nvConfig.back.iconForce)}]" @tap.stop="nvBackTap" :style="{'background':(isSharePage&&!(nvConfig.back&&nvConfig.back.iconForce)?(nvConfig.componentBgColor||''):''),'color':getTxtColor,'border-radius':'26rpx','font-size': isSharePage&&!(nvConfig.back&&nvConfig.back.iconForce)?'36rpx':'54rpx'}" v-else>{{isSharePage&&!(nvConfig.back&&nvConfig.back.iconForce)?'&#xe605;':'&#xe743;'}}</text>
				</block>
...
<script>
	...
	export default {
		...
		computed:{
			...
			//固定定位判断
			isFixed(){
				let position = this.nvConfig.position||{type:'fixed'};
				if(typeof position == "string")position = {type:position}
			...
			async getNvBoxRect(){
				...
				if(this.nvConfig.model){
					if(position.hasOwnProperty("top")){
						this.nvBoxRect = Object.assign(view,{wait:false,top:position.top})
						return
					}
				}else{
					this.nvBoxRect = Object.assign(view,{wait:false,top:position.top||0})
				console.log(this.nvConfig.position,position);
					return
				}
...
<style lang="scss" scoped>
	...
	.nvbackImg{height: auto;padding: 0;left: 20rpx;}
